...
for i in range(len(p)):
    ...
    for j in range(len(D)):
        Dd1 = p[i] * D[j]             # CAV需求
        Dg1 = (1 - p[i]) * D[j]       # HDV需求
        vd1 = vel(Dd1, Nd*cap(1, 1, K), vf)        # CDL虚拟速度
        vg1 = vel(Dg1, (N-Nd)*cap(2, 0, K), vf)    # GL虚拟速度

     #(C, G)的流量分布
        if vd1 >= vg1:  # 当专用车道上的虚拟速度高于通用车道，此时CAV不会选择GL
            Dd = p[i] * D[j]
            Dg = (1 - p[i]) * D[j]
            pd = 1
            pg = 0
            cap_g = (N-Nd)*cap(2, 0, K)  
            alpha = 0
        else:
            pd = 1
            pg = 0
            ...
            rate = 0
            e = 0.01
            diff = vel((p[i] - rate) * D[j],Nd*cap(1, pd, K),vf) - vel((1 - p[i] + rate) * D[j],(N-Nd)*cap(2, pg, K),vf)
            while np.all((abs(diff)) > e):
                rate = (rate_high - rate_low) * 0.5 + rate_low
                pd1 = 1
                pg1 = rate / (1 - p[i] + rate)
                diff = vel((p[i] - rate) * D[j],Nd*cap(1, pd1, K),vf) - vel((1 - p[i] + rate) * D[j],(N-Nd)*cap(2, pg1, K),vf)
                if np.all(diff >= 0):
                    rate_high = rate
                else:
                    rate_low = rate
            ...
            Dg = (1 - p[i] + alpha) * D[j]
            pd = 1
            pg = alpha / (1 - p[i] + alpha)
            cap_g = (N-Nd)*cap(2, pg, K)   
            
        cap_d = Nd*cap(1, pd, K)   # CDL通行能力
        v_d = vel(Dd, cap_d, vf)   # CDL速度
        v_g = vel(Dg, cap_g, vf)   # GL速度
...
df.to_excel('lane3_f_CG.xlsx', index=False)
